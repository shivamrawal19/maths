...
void main() {
    ...
    float p = ( py - x[noOfSamples-1] ) / fabs(x[1]-x[0]);

    printf("P = x - x0 / h\n");
    printf("  = %f - %f / %f\n", py, x[noOfSamples-1], fabs(x[1]-x[0]));
    printf("  = %f\n", p);
    ...
    float finalAns = dy[0][noOfSamples-1];
    float intermediateAns;

    printf("f(% 9.4f ) = %6.4f\n", py, dy[0][noOfSamples-1]);
    for (i = 0; i < noOfSamples-1; i++) {
        printf("              + ");
        for (j = 0; j <= i; j++) {
            if (j==0) {
        ...
        printf(" %6.4f )", dy[i+1][noOfSamples-2-i]);
        intermediateAns *= dy[i+1][noOfSamples-2-i];
